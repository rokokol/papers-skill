...
- Both `pqa ask` and `pqa-evidence` look for passages only in the papers one index search brings up, at most `programs.papers.corpus.searchCount` of them (4 by default), so "no evidence" can mean the query did not surface the paper rather than that no paper in the folder holds it; before concluding absence, rephrase the query or raise `searchCount`
