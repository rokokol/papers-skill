...
- `references/`: which source for which area and how identifiers map to the server's tools, the reading subagent's prompt word for word, the digest template with its bounds, the vault profile schema, and when a corpus needs PaperQA2 instead
